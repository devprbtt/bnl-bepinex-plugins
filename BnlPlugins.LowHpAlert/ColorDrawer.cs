...
namespace BnlPlugins.LowHpAlert
{
    ...
    {
        private static readonly Dictionary<ConfigEntryBase, string> EditBuffer = new Dictionary<ConfigEntryBase, string>();
        ...
        {
            new PresetColor { Name = "Red",    Color = new Color(1f, 0.27f, 0.27f, 1f) },
        ...
        internal static void Draw(ConfigEntryBase entry)
        {
            ...
            string controlName = "bnl_lhp_hex_" + entry.GetHashCode();
            bool isFocused = GUI.GetNameOfFocusedControl() == controlName;

            string bufferValue;
            if (!EditBuffer.TryGetValue(entry, out bufferValue) || !isFocused)
            {
                bufferValue = stored;
                EditBuffer[entry] = stored;
            }
            ...
            Color liveColor;
            if (ColorHelper.TryParseHex(entry.BoxedValue as string ?? stored, out liveColor))
            {
                float alpha = DrawChannel("A", liveColor.a);
                string updatedHex = ColorHelper.ToHex(new Color(liveColor.r, liveColor.g, liveColor.b, alpha));
                if (!string.Equals(updatedHex, entry.BoxedValue as string ?? string.Empty, StringComparison.OrdinalIgnoreCase))
                {
                    entry.BoxedValue = updatedHex;
                    EditBuffer[entry] = updatedHex;
                    GUI.changed = true;
                }
            }
